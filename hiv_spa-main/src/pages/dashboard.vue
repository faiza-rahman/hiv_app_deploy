<template>
  ...
  <v-dialog v-model="editDialog" max-width="500px">
    ...
    <v-table class="mood-table">
      ...
      <tbody>
        <tr>
          <td v-for="(entry, index) in weeklyMood" :key="index" class="text-center mood-cell">
            <div class="mood-content">
                <v-icon v-if="entry.diaryEntry" :color="getMoodColor(entry.mood)" size="40" class="mood-icon">
        ...
        <v-row justify="center" align="center" dense>
          ...
          >
            <v-btn
              ...
              @click="moodData.mood = option.value"
              elevation="0"
              rounded
            >
  ...
  methods: {
    ...
    async saveMoodEntry() {
      try {
        const moodData = {
          ...
          diaryEntry: this.moodData.diary,
          date: this.moodData.date,
        };
        const response = await logMood(moodData);
        console.log("Mood logged successfully:", response);
        this.closeMoodDialog();
        await this.fetchWeeklyMood(); // Refresh the weekly mood data
      } catch (error) {
    ...
    async fetchWeeklyMood() {
      try {
        ...
        this.weeklyMood = response; // Store the weekly mood data
      } catch (error) {
